...
describe('readLlmEnv', () => {
  it('applies DeepSeek-compatible defaults when only the API key is set', () => {
    expect(readLlmEnv({ EXHARNESS_LLM_API_KEY: 'k' })).toEqual({
  ...
  it('honours explicit values', () => {
    expect(
      readLlmEnv({ EXHARNESS_LLM_BASE_URL: 'http://x/v1', EXHARNESS_LLM_API_KEY: 'k', EXHARNESS_LLM_MODEL: 'm' }),
    ).toEqual({ baseUrl: 'http://x/v1', apiKey: 'k', model: 'm' })
  })

  it('reads from process.env when no env is provided', () => {
    const original = process.env.EXHARNESS_LLM_API_KEY
    process.env.EXHARNESS_LLM_API_KEY = 'from-process'
    try {
      expect(readLlmEnv().apiKey).toBe('from-process')
    } finally {
      if (original === undefined) delete process.env.EXHARNESS_LLM_API_KEY
      else process.env.EXHARNESS_LLM_API_KEY = original
    }
  })

  it('throws when the API key is missing or empty', () => {
    expect(() => readLlmEnv({})).toThrow(/EXHARNESS_LLM_API_KEY/)
    expect(() => readLlmEnv({ EXHARNESS_LLM_API_KEY: '' })).toThrow(/EXHARNESS_LLM_API_KEY/)
  })
})

describe('createLlmFromEnv', () => {
  it('returns an openai-compatible provider', () => {
    const provider = createLlmFromEnv({ EXHARNESS_LLM_API_KEY: 'k' })
